...
public class CatalogView extends RelativeLayout {
    ...
    public CatalogView(Context context, AttributeSet attrs) {
        ...
        ui.listView.setAdapter(getCatalogViewAdapter()); //设置列表适配器
    ...
    protected void onDetachedFromWindow() {
        ...
        //注销目录视图适配器
        if (m.catalogViewAdapter != null) {
            getCatalogViewAdapter().unregister();
        }
    }
    ...
    private int getCurrentCatalogPosition() {
        ...
        List<IBookCatalog> catalogList = getBook().getCatalogList(); //获取书目录列表
    ...
    private CatalogViewAdapter getCatalogViewAdapter() {
        if (m.catalogViewAdapter == null) {
            m.catalogViewAdapter = new CatalogViewAdapter(getContext());
        }

        return m.catalogViewAdapter;
    }
    ...
    private class OnListViewGroupExpandListener implements ExpandableListView.OnGroupExpandListener {
        ...
        public void onGroupExpand(int groupPosition) {
            //遍历所有主题项
            for(int i = 0; i < getCatalogViewAdapter().getGroupCount(); i++) {
