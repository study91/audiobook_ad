 ...
 * 音乐媒体播放器
 */
class MusicMediaPlayer implements IMediaPlayer {
     ...
     */
    public MusicMediaPlayer(Context context, String filename) {
        m.context = context;
    ...
    private MediaPlayer getMediaPlayer() {
        if (m.mediaPlayer == null) {
            ...
            m.mediaPlayer.setVolume(option.getMusicLeftVolume(), option.getMusicRightVolume()); //设置音量
    ...
    private class OnMediaPreparedListener implements MediaPlayer.OnPreparedListener {
        ...
        public void onPrepared(MediaPlayer mp) {
            ...
            intent.putExtra(BookMediaService.MESSAGE, BookMediaService.EVENT_MUSIC_PREPARED); //设置消息
    ...
    private class OnMediaCompletionListener implements MediaPlayer.OnCompletionListener {
        ...
        public void onCompletion(MediaPlayer mp) {
            Intent intent = new Intent(getOption().getServiceAction()); //获取媒体服务端Intent
            intent.putExtra(BookMediaService.MESSAGE, BookMediaService.EVENT_MUSIC_COMPLETION); //设置消息
            getContext().sendBroadcast(intent); //发送广播
        }
    }
